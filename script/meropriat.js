document.addEventListener("DOMContentLoaded", function() {
    ...
    publicEvents.forEach(event => {
        ...
        eventItem.innerHTML = `
            <img alt="${event.name} Image" src="${event.imageDataUrl || 'img/edinstvo.WEBP'}" />
            <h3>${event.name}</h3>
            ...
            font-size: 16px;
            border-radius: 5px;" onclick='participateInEvent(${JSON.stringify(event)})'>Принять участие</button> <!-- Кнопка для участия -->
        `;
